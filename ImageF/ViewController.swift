...
UIViewController{
    ...
    private func checkInputValuesToTextBox(inputText:NSString)->NSString{
    ...
    private func drawImageToView(){
        ...
        if kof>1{
            if Int(imageToShow.size.width) > _viewSizeWidht-25{
                _width=_viewSizeWidht-25
                _height=Int(Float(_width)/kof)
            }
        }
        else{
            if Int(imageToShow.size.height) > _viewSizeHeight-70{
                _height = _viewSizeHeight-70
                _width=Int(Float(_height)*kof)
            }
        }
        if (kof)<1.1&&(kof)>0.9{
            if _viewSizeHeight > _viewSizeWidht{
                _width=_viewSizeWidht-75
                _height=_width-1
            }
            else {
                _width=_viewSizeHeight-75
                _height=_width-1
            }
        }
        println("\(_width) \(_height) \(kof)")
        ...
        var myConstraint = NSLayoutConstraint(item: self.imageView, attribute: NSLayoutAttribute.CenterY, relatedBy: NSLayoutRelation.Equal, toItem: self.view, attribute: NSLayoutAttribute.CenterY, multiplier: 1.0, constant: 20)
    ...
    private func makeNewImageWithGoodSize (image:UIImage, _width:Int,_height:Int)->UIImage{
